...
analyze symbol years="5":
    cargo run --package perth-bin -- analyze {{symbol}} --years {{years}}

# Run risk analysis for a stock
# Usage: just risk SYMBOL
# Example: just risk UNH
risk symbol:
    cargo run --package perth-bin -- risk --symbol {{symbol}}

# Show S&P 500 universe information
universe:
    cargo run --package perth-bin -- universe

# Show stocks in a specific sector
# Usage: just sector SECTOR
# Example: just sector healthcare
sector name:
    cargo run --package perth-bin -- universe --sector {{name}}

# List all GICS sectors
sectors:
    cargo run --package perth-bin -- universe --list-sectors
